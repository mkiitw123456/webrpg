...
export function updateForge(ui, now) {
  ...
  const preview=item?`<img class="pixel-icon" src="${itemIcon(item)}" alt="${ITEMS[item].name}"><span>+${level} → +${Math.min(15,level+1)} · ${rules.cost} 金幣 · 持有 ${r.gold}<br>成功率 ${rules.successRate}% · 失敗後爆裝率 ${rules.destroyRate}%<br>${rules.destroyRate?'失敗後以第二輪進度條顯示爆裝判定':'本次沒有爆裝風險'}</span>`:'<p>沒有可強化的裝備。</p>';
