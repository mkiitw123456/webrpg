...
export function updateForge(ui, now) {
  ...
  root.querySelector('#forge-start').disabled=!ui.scene.client?.connected||running||!item||level>=10||r.gold<rules?.cost;
  const preview=item?`<img class="pixel-icon" src="${itemIcon(item)}" alt="${ITEMS[item].name}"><span>+${level} → +${Math.min(10,level+1)} · ${rules.cost} 金幣 · 持有 ${r.gold}<br>成功 +${rules.successMin}–${rules.successMax}（大強化 ×2）／失敗 +${rules.failureMin}–${rules.failureMax}<br>${level>6?'失敗後進入爆裝競速：保留 +8–22／爆裝 +5–18':'本次沒有爆裝風險'}</span>`:'<p>沒有可強化的裝備。</p>';
